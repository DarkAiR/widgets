...
#### Описание постановки на библиотеку виджетов
...
3. **Мы используем TypeScript.** Нативный JS слишком примитивен и его использование необосновано ничем кроме legacy-кода.   
   В случае legacy, портирование кода на TypeScript максимально просто и не должно вызывать никаких проблем кроме лени.   
...
4. **Мы используем препроцессоры.** В нашем случае LESS. В современном мире никто не пишет на голом CSS из-за его очевидных ограничений.   
   Любой препроцессор в конечном итоге транслируется в CSS, из коробки поддерживается псевдоязык программирования, кроссбраузерность и иерархичность.   
...
6. **Мы не держим в проекте никаких лишних файлов стилей**, если это не обосновано и не согласовано с "бизнесом".   
...
8. Любые попытки вмешаться в основную структуру библиотеки, без предварительного согласования, будут отклоняться.
   
**Все коммиты, не соответствующие требованиям выше будут отклоняться и отправляться на доработку.**  
