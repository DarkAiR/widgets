...
import {ChartType, Paddings} from "./types";
...
export class DiagramChart implements IChart {
    ...
    // private yOriginalHeight: number;
    // private yOriginalHeightSecondary: number;
    ...
    render() {
        ...
        this.padding.right               = this.data.properties.secondaryAxis
            ...
            : 0;
        // this.yOriginalHeightSecondary    = this.yHeightSecondary;
        // this.yOriginalHeight             = this.yHeight;
        ...
        this.data.values.forEach((value, i) => {
            console.log('>> ', value.date, this.x(moment(value.date)));
            ...
            DiagramChart.addEventListeners(g.append('rect')
                .attr('class', d => (d.classes ? this.styles[d.classes[0]] : '') + ' ' + (d.interactive ? this.styles['diagram__interactive'] : ''))
            ...
            DiagramChart.addEventListeners(g.append('rect')
                .attr('class', d => (d.classes ? this.styles[d.classes[1]] : '') + ' ' + (d.interactive ? this.styles['diagram__interactive'] : ''))
                .attr('x', (d, i) => (width + margin) * i)
            ...
            g.append('text')
                .attr('class', this.styles['diagram__icon']).text(d => d.icon)
                .attr('x', (d, i) => (width + margin) * i + 2).attr('y', d => this.y(d.value) + 20);
        });
        ...
        //             case 'LINE':
        //                 let g             = this.base.append('g').attr('class', `diagram__line ${indicator.properties.class ? indicator.properties.class : ''}`);
    ...
    // getDomain(axis) {
    //     if (axis === 'x') return this.xAxisDomain;
    //     if (axis === 'y') return this.yDomain'];
    // }
