...
/**
        ...
        if (data.dataSets[0].settings.valueType !== undefined && data.dataSets[0].settings.subValue !== undefined && data.dataSets[0].settings.valueType === 'text') {
            valueMain = _get(data.dataSets[0].settings, 'subValue', '');
        } else {
            valueMain = _get(data.data[0], '[0].value', 0);
        }
        let valueSub;
        let valueSubColor;
        let style2;
        ...
        if (data.data[1] !== undefined) {
            ...
            if (data.dataSets[1].settings.valueType !== undefined && data.dataSets[1].settings.subValue !== undefined && data.dataSets[1].settings.valueType === 'text') {
                valueSub = _get(data.dataSets[1].settings, 'subValue', '');
            } else {
            ...
            value2str = `
            <div class='${w['sub']} ${w['text']} ${w[valueSubColor.className]} ${s["col-vbot"]}'
                 style='${style2}'>
                ${valueSub}
            </div>
            `;
