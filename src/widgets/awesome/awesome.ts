...
import {AwesomeSettings} from './awesomeSettings';
import {
...
export class Awesome extends Chart {
    ...
    run(data: IChartData): void {
        console.log('%cAwesome run', 'color: #b080ff');
        const settings = <AwesomeSettings>data.settings;
    ...
    private getComparedSeries(data: IChartData, timeSeriesData: TimeSeriesData): Object[] {
        ...
        const dates = _map(timeSeriesData.dates, v => new Date(v).getDate());
    ...
    private getYAxis(
        ...
        for (let idx = 0; idx < data.data.length; idx++) {
            if (data.dataSets[idx].settings.createYAxis !== true) {
                continue;
            }
            ...
            if (data.dataSets[idx].settings.yAxisInds !== undefined && data.dataSets[idx].settings.yAxisInds.length > 0) {
                for (const v of data.dataSets[idx].settings.yAxisInds) {
                    ...
                    timeSeriesData.values[v].forEach((item) => {
                        if (item === undefined) {
                            return;
                        }
                ...
                timeSeriesData.values[idx].forEach((item) => {
                    if (item === undefined) {
                        return;
                    }
                    valueArray.push(item);
            ...
            const maxDataValue = Math.max.apply(null, valueArray) + '';

            const roundHB = function(possibleHB: string) {
                ...
                return (possibleHB[0] + nullsStr) === possibleHB ? possibleHB : Number(firstSigns + nullsStr);
            };

            // tslint:disable-next-line: no-any
            const checkNums = function(num: any, lastMeaningDigitInd: number) {
                if (num[lastMeaningDigitInd] !== '10' || lastMeaningDigitInd === 0) {
                    return num;
                } else if (lastMeaningDigitInd !== 0 && num[lastMeaningDigitInd] === '10') {
                    num[lastMeaningDigitInd] = '0';
                    num[lastMeaningDigitInd - 1] = String(Number(num[lastMeaningDigitInd - 1]) + 1);
                    return checkNums(num, lastMeaningDigitInd - 1);
                }
            };

            // tslint:disable-next-line: no-any
            const magicRound = function(hb: any) {
                if (isNaN(hb)) {
                    return;
                } else if (hb % 3 === 0) {
                    return hb;
                    ...
                    hb += '';
                    hb = hb.split('');
                    let lastMeaningDigitInd;

                    if (hb.length > 2) {
                        lastMeaningDigitInd = hb.length - 3;
                    } else {
                        lastMeaningDigitInd = hb.length - 1;
                    }

                    hb[lastMeaningDigitInd] = String(Number(hb[lastMeaningDigitInd]) + 1);
                    hb = checkNums(hb, lastMeaningDigitInd);
                    hb = Number(hb.join(''));
                    return magicRound(hb);
                }
            };

            const higherBorder = roundHB(maxDataValue);
            const finalHB = magicRound(higherBorder) < Math.ceil(+maxDataValue) ? magicRound(roundHB(String(Math.ceil(+maxDataValue)))) : magicRound(higherBorder);
